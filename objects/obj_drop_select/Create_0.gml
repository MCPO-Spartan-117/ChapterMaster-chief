...
if (p_target.p_player[planet_number] > 0) { max_ships += 1; }

if (purge == 0) {
    var spesh = false;
    if (planet_feature_bool(p_target.p_feature[planet_number], P_features.Warlord10) == 1) && (obj_controller.faction_defeated[10] == 0) && (obj_controller.faction_gender[10] == 1) && (obj_controller.known[eFACTION.Chaos] > 0) && (obj_controller.turn >= obj_controller.chaos_turn) { spesh=true; }

    var bes = 0, bes_score = 0;
    if (sisters > 0) && (obj_controller.faction_status[eFACTION.Ecclesiarchy] = "War"){bes = 5; bes_score = sisters;}
    ...
    target = 2;
    if (eldar > csm) && (eldar > traitors) && (eldar > ork) && (eldar > tau) && (eldar > tyranids) && (eldar > necrons) { target = 6; }
    if (ork > csm) && (ork > traitors) && (ork > eldar) && (ork > tau) && (ork > tyranids) && (ork > necrons) { target = 7; }
    if (tau > csm) && (tau > traitors) && (tau > eldar) && (tau > ork) && (tau > tyranids) && (tau > necrons) { target = 8; }
    if (tyranids > csm) && (tyranids > traitors) && (tyranids > ork) && (tyranids > tau) && (tyranids > eldar) && (tyranids > necrons) { target = 9; }
    if (csm > ork) && (csm >= traitors) && (csm > eldar) && (csm > tau) && (csm > tyranids) && (csm > necrons) { target = 10; }
    if (traitors > ork) && (traitors >= csm) && (traitors > eldar) && (traitors > tau) && (traitors > tyranids) && (traitors > necrons) { target = 10; }
    if (necrons > ork) && (necrons >= csm) && (necrons > eldar) && (necrons > tau) && (necrons > tyranids) && (necrons > traitors) { target = 13; }
    if (p_target.p_owner[obj_controller.selecting_planet] == 8) {
        if (pdf > csm) && (pdf > traitors) && (pdf > eldar) && (pdf > ork) && (pdf > tyranids) && (pdf > tau) && (pdf > necrons) { target = 2.5; }
    }
    if (p_target.craftworld == 1) { target = 6; }
}

var _fleet_bombard_score = calculate_fleet_bombard_score(sh_target.fleet_ships);
var _viable_ground_forces = calculate_fleet_content_size(sh_target.fleet_ships);
bombard_purge = new PurgeButton(4, 631,231,DropType.PurgeBombard);
bombard_purge.active = _fleet_bombard_score ? 1:0;
